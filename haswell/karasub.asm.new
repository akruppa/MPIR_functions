...
dnl  void mpn_karasub(mp_ptr, mp_ptr, mp_size_t)
dnl  rax                 rdi     rsi        rdx
dnl  rax                 rcx     rdx         r8
dnl
dnl  Karasuba Multiplication - split x and y into two equal length halves so
dnl  that x = xh.B + xl and y = yh.B + yl. Then their product is:
dnl
dnl  x.y = xh.yh.B^2 + (xh.yl + xl.yh).B + xl.yl
dnl      = xh.yh.B^2 + (xh.yh + xl.yl - {xh - xl}.{yh - yl}).B + xl.yl
dnl
dnl If the length of the elements is m (about n / 2), the output length is 4 * m 
dnl as illustrated below.  The middle two blocks involve three additions and one 
dnl subtraction: 
dnl 
dnl       -------------------- rp
dnl       |                  |-->
...
dnl       --------------------
dnl
dnl To avoid overwriting B before it is used, we need to do two operations
dnl in parallel:
dnl
dnl (1)   B = B + C + A - E = (B + C) + A - E
dnl (2)   C = C + B + D - F = (B + C) + D - F
dnl
dnl The final carry from (1) has to be propagated into C and D, and the final
dnl carry from (2) has to be propagated into D. When the number of input limbs
dnl is odd, some extra operations have to be undertaken. 
